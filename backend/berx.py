...
BERX_META = {"symbol": "BERX", "name": "Berx Token"}


async def seed_berx(db):
...
async def set_price(db, new_price: float):
    new_price = max(0.0001, float(new_price))
    await push_tick(db, new_price)
    return new_price


async def adjust_price(db, pct: float):
    cur = await get_berx_price(db)
    new_price = cur * (1 + (pct / 100.0))
    new_price = max(0.0001, new_price)
    await push_tick(db, new_price)
    return new_price


async def get_ticker(db) -> dict:
    cur = await get_berx_price(db)
    # 24h ago reference
    day_ago = (datetime.now(timezone.utc) - timedelta(hours=24)).isoformat()
    past = await db.berx_ticks.find_one({"ts": {"$lte": day_ago}}, sort=[("ts", -1)])
    past_price = float(past["price"]) if past else cur
    change = ((cur - past_price) / past_price * 100.0) if past_price else 0.0
    # 24h high/low
...
async def get_klines(db, interval: str = "1h", limit: int = 200) -> list[dict]:
    ...
    since = (datetime.now(timezone.utc) - timedelta(seconds=bucket_seconds * limit * 2)).isoformat()
    ticks = await db.berx_ticks.find(
        {"ts": {"$gte": since}}, {"_id": 0}
    ...
    for t in ticks:
        ...
        b["low"] = min(b["low"], p)
        b["close"] = p
    rows = [{"time": k, **v} for k, v in sorted(buckets.items())]
    return rows[-limit:]


async def get_sparkline(db, points: int = 24) -> list[float]:
    candles = await get_klines(db, interval="1h", limit=points)
    return [c["close"] for c in candles]


async def get_depth(db) -> dict:
    cur = await get_berx_price(db)
    bids = []
    asks = []
    for i in range(1, 11):
...
async def get_recent_trades(db) -> list[dict]:
    cur = await get_berx_price(db)
