...
const COLORS = ["#DCA335", "#10B981", "#3B82F6", "#8B5CF6", "#EC4899", "#F59E0B", "#06B6D4"];
  ...
  return (
    <div className="p-6 lg:p-8 max-w-[1440px] mx-auto anim-fade-up">
      <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
        <div>
          <div className="text-sm text-[#94A3B8]">Toplam Portföy Değeri</div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="card-surface p-5 lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <div>
              <div className="text-xs text-[#94A3B8]">BTC/TRY</div>
              <div className="font-display text-xl tabular">{formatTRY(markets.find((m) => m.symbol === "BTC")?.price_try ?? 0)}</div>
            </div>
            <div className={`text-sm tabular ${markets.find((m) => m.symbol === "BTC")?.change_24h >= 0 ? "text-[#10B981]" : "text-[#EF4444]"}`}>
              ...
              <AreaChart data={klines}>
                <defs>
                  <linearGradient id="goldFill" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#DCA335" stopOpacity={0.35} />
                ...
                <Tooltip
                  contentStyle={{ background: "#11151E", border: "1px solid #1F2633", borderRadius: 8 }}
                  labelFormatter={(t) => new Date(t * 1000).toLocaleString("tr-TR")}
                  formatter={(v) => formatTRY(v)}
                />
                <Area type="monotone" dataKey="close" stroke="#DCA335" fill="url(#goldFill)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="space-y-1 mt-2">
            {pieData.map((a, i) => (
              <div key={a.symbol} className="flex items-center justify-between text-xs">
                ...
                <span className="tabular text-[#94A3B8]">{formatTRY(a.value_try)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mt-6">
        <div className="card-surface p-5">
          <div className="text-xs text-[#94A3B8] mb-3">Piyasada Öne Çıkanlar</div>
          <div className="divide-y divide-[#1F2633]">
            {markets.slice(0, 6).map((m) => (
              <Link to={`/trade/${m.symbol}`} key={m.symbol} className="flex items-center justify-between py-3 hover:bg-[#1A202C] px-2 -mx-2 rounded" data-testid={`dash-market-${m.symbol}`}>
                <div className="flex items-center gap-3">
                ...
                <div className="text-right">
                  ...
                  <div className={`tabular text-xs flex items-center justify-end gap-1 ${m.change_24h >= 0 ? "text-[#10B981]" : "text-[#EF4444]"}`}>
        ...
        <div className="card-surface p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="text-xs text-[#94A3B8]">Son İşlemler</div>
            <Link to="/history" className="text-xs text-[#DCA335]" data-testid="dash-history-link">Tümü</Link>
          </div>
          {txs.length === 0 ? (
            <div className="text-sm text-[#94A3B8] py-6 text-center">İşlem kaydı yok</div>
          ) : (
            <div className="divide-y divide-[#1F2633]">
              {txs.map((t, i) => (
                <div key={i} className="flex items-center justify-between py-3">
                  <div>
                    ...
                    <div className="text-xs text-[#94A3B8]">{new Date(t.created_at).toLocaleString("tr-TR")}</div>
                  </div>
