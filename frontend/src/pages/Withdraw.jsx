...
export default function Withdraw() {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-3xl mx-auto anim-fade-up">
      ...
      <p className="text-[#94A3B8] text-sm mb-6">IBAN'ınıza havale ile gönderilir. KYC onayı gereklidir.</p>

      <div className="card-surface p-4 mb-4 flex items-center justify-between">
        <span className="text-xs text-[#94A3B8]">Çekilebilir Bakiye</span>
      ...
      <form onSubmit={submit} className="card-surface p-6 mb-6" data-testid="withdraw-form">
        ...
        <label className="text-xs text-[#94A3B8]">IBAN (TR ile başlamalı)</label>
      ...
      <div className="card-surface p-6">
        <div className="text-xs text-[#94A3B8] mb-3">Çekim Geçmişiniz</div>
        {list.length === 0 ? <div className="text-sm text-[#94A3B8] py-4">Henüz çekim yok</div> : (
          <table className="w-full text-sm">
            <thead><tr className="text-xs text-[#94A3B8] text-left"><th>Tarih</th><th>Tutar</th><th>IBAN</th><th>Durum</th></tr></thead>
            <tbody className="divide-y divide-[#1F2633]">
              {list.map((w) => (
                <tr key={w.withdrawal_id}>
                  ...
                  <td className="text-xs tabular text-[#94A3B8]">{w.iban}</td>
                  <td className={w.status==="approved"?"text-[#10B981]":w.status==="rejected"?"text-[#EF4444]":"text-[#F59E0B]"}>{{approved:"Onaylandı",rejected:"Reddedildi",pending:"Beklemede"}[w.status] || w.status}</td>
                </tr>
              ))}
            </tbody>
          </table>
